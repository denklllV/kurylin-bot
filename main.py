...
async def main() -> None:
    ...
    # Используем токен первого клиента для инициализации, но это не принципиально
    ...
    # Заполняем `application.bot_data` общими сервисами
    ...
    if RUN_MODE == 'WEBHOOK':
        logger.info("Setting webhooks for all clients...")
        ...
        logger.info(f"Starting single webhook server on port {PORT}...")
        await app.run_webhook(
            listen="0.0.0.0",
            port=PORT,
            # Этот url_path используется для установки вебхука для основного бота,
            # но наш middleware будет перехватывать все запросы
            url_path=clients[0]['bot_token'],
            webhook_url=PUBLIC_APP_URL
        )
    else: # POLLING
        logger.info("POLLING mode is not supported in this multi-tenant setup. Please use WEBHOOK mode.")
