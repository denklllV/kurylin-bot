...
def get_ai_response(question: str) -> str:
    ...
    system_prompt = (
        ...
        " Всегда придерживайся экспертного, но доброжелательного и уверенного тона, без давления. "
        "Никогда не упоминай слова 'контекст', 'база знаний', 'AI', 'модель' или 'цитата'. "
        "Никогда не представляйся, если не спросили напрямую. Сразу переходи к сути. "
        "Используй HTML-теги для форматирования: <b>...</b> для жирного, <i>...</i> для курсива."
    )
    ...
    try:
        ...
        # Добавляем ссылку на источник, только если была использована цитата
        if quote_item:
