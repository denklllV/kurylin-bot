...
from .config import logger, SUPABASE_URL, SUPABASE_KEY, GOOGLE_SHEET_ID
...
def get_leads_for_period(start_date, end_date):
    ...
    try:
        ...
        response = supabase.table('leads').select('*, users(utm_source)').gte('created_at', start_date.isoformat()).lt('created_at', end_date_inclusive.isoformat()).execute()
...
def export_to_google_sheets(start_date_str=None, end_date_str=None):
    ...
    sheet = get_sheet()
    if not sheet:
        return "Ошибка: Не удалось подключиться к Google Таблице. Проверьте ключ и права доступа."
